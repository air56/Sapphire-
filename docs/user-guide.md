...
本次版本以 2026 年 9 月 11 日可正常显示歌词的滚动逻辑为基础，保留透明视觉样式；同时移除了歌词渲染对 `Element.replaceChildren()` 的依赖，兼容 Sapphire 5 的旧版 Qt WebEngine。

本次 `0.1.13` 在 `0.1.9` 的歌词滚动修复基础上，加入了透明文字层视觉预览：可独立调整宽度、高度、主歌词字号、翻译字号、间距、翻译开关，以及中文、日文/假名、英文/拉丁三组字体；同时修复了预览 iframe 缓存和 Referrer 缺失时误连真实 Bridge 的问题。请删除旧组件后重新导入带版本号的新包。
...
生成的文件为 `dist/netease-lyrics-glass-v0.1.13.sawidget`。本次修复后的组件版本为 `0.1.13`。在 Sapphire 的小组件导入界面选择这个文件，然后将组件添加到桌面。
...
| --- | --- | --- | --- | --- |
| 待补充 | Windows 10 | 待补充 | 0.1.13 | 待验证 |
| 待补充 | Windows 11 | 待补充 | 0.1.13 | 待验证 |