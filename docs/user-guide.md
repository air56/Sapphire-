...
本次 `0.1.2` 修复了 Sapphire 在页面启动初期尚未填充 `smtcMediaInfo` / `smtcPlaybackStatus` 时，组件过早读取属性而错过当前歌曲的问题。请删除旧组件后重新导入新包，避免 Sapphire 继续使用旧版本缓存。
...
生成的文件为 `dist/netease-lyrics-glass.sawidget`。在 Sapphire 的小组件导入界面选择这个文件，然后将组件添加到桌面。
...
| --- | --- | --- | --- | --- |
| 待补充 | Windows 10 | 待补充 | 0.1.0 | 待验证 |
| 待补充 | Windows 11 | 待补充 | 0.1.0 | 待验证 |
