...
本次 `0.1.3` 修复了网易云歌词接口返回格式变化，以及 Windows SMTC 未提供歌曲时长时的重复歌曲误判。`0.1.2` 修复了 Sapphire 在页面启动初期尚未填充 `smtcMediaInfo` / `smtcPlaybackStatus` 时，组件过早读取属性而错过当前歌曲的问题。请删除旧组件后重新导入新包，避免 Sapphire 继续使用旧版本缓存。
...
生成的文件为 `dist/netease-lyrics-glass.sawidget`。本次修复后的组件版本为 `0.1.3`。在 Sapphire 的小组件导入界面选择这个文件，然后将组件添加到桌面。
...
| --- | --- | --- | --- | --- |
| 待补充 | Windows 10 | 待补充 | 0.1.3 | 待验证 |
| 待补充 | Windows 11 | 待补充 | 0.1.3 | 待验证 |
