...
本次 `0.1.14` 在 `0.1.13` 的歌词滚动修复基础上，将正式组件改为纯透明文字层：移除彩色环境光、卡片底色、组件边框和组件阴影；同时关闭新实例的 Sapphire 背景与侧栏效果。预览页仍保留独立的示例背景，便于挑选字体和文字效果。请删除旧组件后重新导入带版本号的新包。
...
- 正式组件使用纯透明背景：歌词直接悬浮在壁纸上，不绘制亚克力底色、背景模糊、卡片边框或组件阴影；文字保留轻微可读性阴影。
...
| --- | --- | --- | --- | --- |
| 待补充 | Windows 10 | 待补充 | 0.1.14 | 待验证 |
| 待补充 | Windows 11 | 待补充 | 0.1.14 | 待验证 |
